...
import { _getInputTextSize } from 'nvd-js-helpers/dom-helper'
...
import AlertCircleIcon from 'nvd-u/icons/AlertCircle.vue'
import CheckIcon from 'nvd-u/icons/Check.vue'
import CheckCircleIcon from 'nvd-u/icons/CheckCircle.vue'
...
<template>
    <div ref="el" class="apm-editable d-flex align-items-center gap-2 flex-wrap">
        <slot :value="_value">
            <UTextarea v-if="type === 'textarea'" v-model="_value" placeholder="not set"/>
            <USwitch v-model="_value" v-else-if="type === 'switch'"/>
            <input v-else :type="type" v-model="_value" placeholder="not set" @keypress.enter="ok"/>
        </slot>
